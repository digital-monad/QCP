...
def QEC():
    '''runs a demonstration of quantum error correction on a 9 qubit circuit'''
    print("Start with a 9 qubit register prepared in state |0>")
    gate = choice(['X','Z'])
    print(f"Choose a random gate to erroneuosly apply in the circuit : {gate}")
    pos = choice([0,1,2,3])
    print(f"Choose random bit to apply to : {pos}")
# Initialise a 9 qubit quantum register
    ...
    errorGate = eg * gates.I**(9-pos-1)
    errorGate = gates.I**pos * errorGate if pos > 0 else errorGate
    ...
    print("Perform a measurement to hopefully extract what the erroneous gate was and where it occured")
    measurement = bin(qState.MeasureAll())[-4:]
    print("The error gate was the " + measurementToGate[measurement])
